...
class Services
{
	...
	private $walk_step = 0;
	private $item_price_arry = array();
	private $item_arry = array();

	const DB_TABLE = 'szolgaltatas_extra';
	...
	public function getList( $arg = array() )
	{
		...
		if( isset($this->filters['in_ids']) && !empty($this->filters['in_ids']) ) {
			if( is_array($this->filters['in_ids']) && count($this->filters['in_ids']) > 0 ) {
				$q .= " and s.szolg_id IN (\"".implode('","',$this->filters['in_ids'])."\") ";
			}			
		}

		// Csak hirdetésekhez kapcsolódó hirdetések listázása
		if( $arg['only_for_ads'] === true ){
			$q .=  " and s.hirdeteshez = 1 ";
		}

		// Csak hirdetésekhez kapcsolódó hirdetések listázása
		if( $arg['not_for_ads'] === true ){
			$q .=  " and s.hirdeteshez = 0 ";
		}
		...
		foreach ( $qry_data as $service ) {
			...
			$this->item_price_arry[$item['szolg_id']] = array( 
				'price' 		=> $item['ar'],
				'discounted' 	=> ($item['kedvezmeny_szazalek'] != 0) ? $item['kedvezmeny_szazalek'] : false
			);
			$this->item_arry[$item['szolg_id']] = $item;
	...
	{
		$data = $this->db->query("SELECT * FROM ".self::DB_TABLE." WHERE szolg_id = '$id';")->fetch(\PDO::FETCH_ASSOC);
	...
	{
		if( $this->current_item['hirdeteshez'] == '1' ) {
			return true;
		} else {
			return false;
		}
	...
	{
		return $this->item_arry[$item_id];
	}

	public function getItemPrice( $item_id, $with_vat = false )
	{
		$price = (float)$this->item_price_arry[$item_id]['price'];

		if( !$price ) return 0;

		// Kedvezmény leszámolás
		$kedvezmeny = $this->item_price_arry[$item_id]['discounted'];
		if( $kedvezmeny ) {
	...
	public function getID()
	{
		...
		return  $this->current_item['szolg_id'];
	}
	...
	/**
	 * A csomagban engedélyezett futamidó
	 * */
	public function getAllowedDay()
	{
		$days = $this->current_item['ervenyes_napig'];
		return $days;
	}
	 ...
	 * */
	public function getPrice( $with_vat = false )
	{
		
		$price = $this->current_item['ar'];
	...
	public function getStandardPrice( $with_vat = false )
	{
	...
	public function isOrszagos( )
	{
		return ($this->current_item['vmod'] == 'orszagos') ? true : false;
	}
	...
	public function __destruct()
	{
		...
		$this->total_items = 0;
		$this->walk_step = 0;
		$this->item_price_arry = array();
		$this->item_arry = array();
	}
}
?>